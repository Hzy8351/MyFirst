...
namespace FXFramework
{
    public interface IObjectPoolSystem : ISystem
    {
        ...
        void Get(string name,string abName, Action<GameObject> callBack = null);
    ...
    public class ObjectPoolSystem : AbstractSystem, IObjectPoolSystem
    {
        ...
        //重载--AB加载
        void IObjectPoolSystem.Get(string name,string abName, Action<GameObject> callBack)
        {
            //如果有对应的格子 如果格子有东西 就取出来
            if (mPoolDic.TryGetValue(name, out PoolData data) && data.CanGet)
            {
            ...
            //异步加载资源 创建对象给外部用 如果回调函数不为空 则抛出该对象
            ResHelper.AsyncLoadAB<GameObject>(name,abName, o =>
            {
                o.name = name;
                callBack?.Invoke(o);
            });
